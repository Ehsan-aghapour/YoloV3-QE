...
bool Explicit_channels_reorder=true;
rknn_context ctx = 0;
...
void print_output(rknn_output output,int n){
	...
	printf("Max is %d:%f",max_index,max);
}
...
int main(int argc, char *argv[]){
	...
	std::cerr<<"model reading done "<<std::chrono::duration_cast<std::chrono::duration<double>>(t_end-t_start).count()<<std::endl;
	...
	std::cerr<<"initialized "<<std::chrono::duration_cast<std::chrono::duration<double>>(t_end-t_start).count()<<std::endl;
	...
	if (argc==4){
		...
		std::cerr<<"\n\nreading image "<<img_path<<std::endl;
		t_start=std::chrono::high_resolution_clock::now();
		rockx_image_read(img_path, &input_image, 1);
		t_end=std::chrono::high_resolution_clock::now();
		std::cerr<<"image read done "<<std::chrono::duration_cast<std::chrono::duration<double>>(t_end-t_start).count()<<std::endl;
		...
		//float test_data[input_size];
		float *test_data = new float[input_size]();
		
		
		
		if(Explicit_channels_reorder){
		...
		*/
		...
		if(Explicit_mean_reduction){
			...
			for (int j=0;j<input_size;j+=3){
				...
				if(input_image.pixel_format==2){
					//std::cerr<<"\n\n\n\n\n\n\n\n\n\n\n";
					test_data[j]=(test_data[j]-mean[0]);
					test_data[j+1]=(test_data[j+1]-mean[1]);
					test_data[j+2]=(test_data[j+2]-mean[2]);
				}
				if(input_image.pixel_format==1){
					test_data[j]=(test_data[j]-mean[2]);
					test_data[j+1]=(test_data[j+1]-mean[1]);
					test_data[j+2]=(test_data[j+2]-mean[0]);
				}
	...
	std::cerr<<"\n\nSetting input into the model...\n";
	t_start=std::chrono::high_resolution_clock::now();
	ret = rknn_inputs_set(ctx, 1, inputs);
	t_end=std::chrono::high_resolution_clock::now();
	std::cerr<<"Warmup: input has set duration: "<<std::chrono::duration_cast<std::chrono::duration<double>>(t_end-t_start).count()<<std::endl;
	...
	printf("salam,%f,%f,%f",out[811],out[812],out[813]);
